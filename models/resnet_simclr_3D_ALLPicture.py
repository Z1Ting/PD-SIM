...
from resnet_3D_ALLPicture import generate_model
import torch
...
class ResNetSimCLR_3D(nn.Module):
    ...
    def __init__(self, base_model, out_dim):
        ...
        del resnet_list[0]  # 删除对象 删除空间注意力模块
        resnet_list[0] = nn.Conv3d(1, 64, kernel_size=3, stride=3, padding=1, bias=False)  #第一个参数原为3channels，灰质图像用1channel
        # resnet_list[1] = nn.Conv3d(1, 64, kernel_size=3, stride=5, padding=1, bias=False)  #第一个参数原为3channels，灰质图像用1channel
        # 在allpicture——attention中，stride=5，这样bz可以=24 ，其余stride=3

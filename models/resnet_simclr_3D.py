...
class ResNetSimCLR_3D(nn.Module):
    ...
    def __init__(self, base_model, out_dim):
        ...
        resnet_list[0] = nn.Conv3d(1, 64, kernel_size=3, stride=1, padding=1, bias=False)  #第一个参数原为3channels，灰质图像用1channel
...
if __name__ == "__main__":
    ...
    print(test)
    input_x = torch.randn((30, 16, 30, 25, 25, 25)) # 第1维由自己定义的resnet的n_input_channels等于几以及     ((10, 1, 2, 2, 2))
                                            # 这里的resnet_list[0] = nn.Conv3d(1, 64, kernel_size=3, stride=1, padding=1, bias=False)  #第一个参数原为3channels，灰质图像用1channel（这里可以绝对决定）
                                            # 一起决定，这里设置为1，因为是灰质图象
    mean_input = input_x.mean(0)
    output_x1,output_x2 = test(mean_input)
